...
function solveTask (task, lookupDepth, shouldLog) {
  return task.sourceSeeds.map(function (seed) {
    return solveSeed(task, seed, lookupDepth, shouldLog);
  });
}

function solver(problems, lookupDepth, shouldLog) {
  return problems.map(function (task) {
    return solveTask(task, lookupDepth, shouldLog);
  });
}
